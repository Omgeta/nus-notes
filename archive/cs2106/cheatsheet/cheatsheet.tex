...
System calls are API calls to the OS for kernel services, changing from user mode to kernel mode via TRAPs.
...
\vspace{-1em}
\begin{enumerate}[\roman*.]
  \item \lstinline|mutex|: syncs readers, \lstinline|empty|: sync readers, writers 
  \item Only the first and last reader will update \lstinline|empty|
\end{enumerate}\vspace{-1pt}
\begin{enumerate}[$-$]
...
\textbf{Variable-Size Partition} have process-dependent partitions. Allocation requires OS to maintain a list/bitmap of partitions and holes.
\begin{enumerate}[$+$]
...
\textbf{Buddy Allocation} splits memory into blocks of size $2^k$. 

\begin{enumerate}[$+$]
  \item Easy coalescing (flip bit $k$ for buddy)
\end{enumerate}\vspace{-1pt}
\begin{enumerate}[$-$]
  \item Internal fragmentation
  \item Requires bookkeeping to track block sizes
\end{enumerate}
\vspace{-1em}
\colbreak
...
Translation Lookaside Buffer (TLB) caches page table entries to avoid 2 slow memory lookups per reference (get frame number + access actual item). \\On context switch, TLB is flushed and new PT brought in.
\begin{enumerate}[\roman*.]
...
\end{enumerate}\vspace{-1pt}
\begin{enumerate}[$-$]
  \item Hard to implement: keep track of last access 
    \begin{itemize}[leftmargin=*]\vspace{2pt}
      \item Counter: maintain logical time counter, incremented on every memory reference; store time into PTE on every page reference.
        \begin{enumerate}[leftmargin=*, label=$-$]\vspace{2pt}
          \item Need to search through all pages 
          \item "Time of use" always increases; can overflow
        \end{enumerate}
      \item Stack: maintain stack of page numbers, remove page from stack and push it to top on reference, then replace page at bottom of stack
        \begin{enumerate}[leftmargin=*, label=$-$]\vspace{2pt}
          \item Not pure stack; hard to implement hardware 
        \end{enumerate}
    \end{itemize}
\end{enumerate}
...
\subsubsection{Common Sizes}
Byte Sizes:
\begin{enumerate}[\roman*.]
  \item $1$ GiB $= 2^{30}$ Bytes
  \item $1$ MiB $= 2^{20}$ Bytes
  \item $1$ KiB $= 2^{10}$ Bytes
\end{enumerate}
...
\begin{enumerate}[\roman*.]
  ...
  \item Per-process: one file descriptor in PCB per open file by process, each to the system-wide table 
    \begin{itemize}[leftmargin=*]\vspace{2pt}
...
\textbf{Linked List (LL)} stores each disk block in nodes (next block number, data), with first, last block number per file.
\begin{enumerate}[$+$]
  \item No external fragmentation
\end{enumerate}\vspace{-1pt}
\begin{enumerate}[$-$]
  \item Slow random access: must traverse linked list
  \item Extra pointer per block; unreliable if pointer wrong
\end{enumerate}
...
\textbf{Indexed Allocation (inode)} uses one "index" block per file to store pointers to all other blocks.
\begin{enumerate}[$+$]
...
\begin{enumerate}[\roman*.]
  ...
  \item DAG: possible when files are shared/linked \vspace{3pt}
    \begin{itemize}[leftmargin=*]
  ...
  \item General: possible when symbolic links create cycle\vspace{1pt} 
    \begin{enumerate}[$-$]
      \item Hard to traverse (need stop infinite loop) 
...
\end{enumerate}\vspace{-1pt}
\begin{enumerate}[$-$]
  \item High overhead: mitigate by storing free block list itself in free blocks
\end{enumerate}

...
Karatsuba Multiplication multiplies two $n$-digit integers:
\begin{enumerate}[\roman*.]
  \item Split $x = x_1\cdot 10^{n/2} + x_0$, $y = y_1\cdot 10^{n/2} + y_0$.
  \item Compute three products: $p_0 = x_0y_0$, $p_2 = x_1y_1$, $p_1 = (x_0{+}x_1)(y_0{+}y_1) - p_0 - p_2$.
  \item Recombine: $xy = p_2\cdot 10^{n} + p_1\cdot 10^{n/2} + p_0$.
  \item $T(n) = 3T(n/2) + \Theta(n) = \Theta\!\bigl(n^{\log_2 3}\bigr) \approx \Theta(n^{1.585})$
\end{enumerate}
...
Dividing function $T(n) = aT(n/b) + f(n)$ where $a>1, b>1, d = \log_ba$ has cases:
\begin{enumerate}[\roman*.]
  \item $f(n) \in O(n^{d-\varepsilon}) \implies T(n)=\Theta(n^{d})$
  \item $f(n) \in \Theta(n^d\log^p n)$ \\
    $\hspace{3em}\land\hspace{0.25em}p>-1 \implies T(n) = \Theta(n^d\log^{p+1}n)$\\
    \hspace{3em}$\land\hspace{0.25em}p=-1 \implies T(n) = \Theta(n^d\log\log n)$\\
    \hspace{3em}$\land\hspace{0.25em}p<-1 \implies T(n) = \Theta(n^d)$
...
\begin{enumerate}[\roman*.]
  ...
  \item Optimal Substructure: optimal solution can be constructed from optimal solutions of subproblems
    \begin{itemize}[leftmargin=*]\vspace{3pt}
      \item Cut-and-paste Proof: Suppose not, i.e. exists "optimal" solution $S$. If we fix a choice, the remainder must be an optimal solution to the smaller instance. Otherwise, replacement by optimal subsolution $S'_{opt}$ strictly improves solution to $S_{opt}$ $\rightarrow$ contradicts optimality of $S$, justifying optimal substructure.
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}[\roman*.]
  ...
  \item NP-Hard: if for every problem $ X \in$ NP, $X \preduce A$
    \begin{itemize}[leftmargin=*]\vspace{2pt}
      \item Proof: can show $p(n)$-time reduction from known NP-Complete problem
    \end{itemize}
...
SUBSET-SUM checks if a multiset $S$ of integers has a subset summing exactly to target $T$:
\begin{enumerate}[\roman*.]
  \item NP: given subset $X \subseteq S$, compute $\sum_{x\in X} x$ and compare to $T$ in $O(|S|)$.
  \item NP-Hard: encode variables/clauses of a 3-SAT instance as digits in a large base so that satisfying assignments correspond to subsets whose sum is $T$; hence 3-SAT $\preduce$ SUBSET-SUM.
\end{enumerate}

\colbreak
PARTITION checks if a multiset $S$ can be split into $S_1,S_2$ with equal total weight:
\begin{enumerate}[\roman*.]
  \item NP: verify $S_1 \cup S_2 = S$, $S_1 \cap S_2 = \varnothing$, and $\sum_{x\in S_1}x = \sum_{x\in S_2}x$ in $O(|S|)$.
  \item NP-Hard: transform SUBSET-SUM instance $(S,T)$ into PARTITION by adding carefully chosen extra elements so that a subset summing to $T$ exists iff the new multiset can be partitioned; hence SUBSET-SUM $\preduce$ PARTITION.
\end{enumerate}

Hamiltonian Cycle (HAM-CYCLE) checks if $G=(V,E)$ has a simple cycle visiting every vertex exactly once:
\begin{enumerate}[\roman*.]
  \item NP: given cycle $C$, verify that every vertex in $V$ appears exactly once and all edges of $C$ lie in $E$ in $\Theta(|V|+|E|)$.
  \item NP-Hard: reduce 3-SAT to HAM-CYCLE using variable and clause gadgets so that satisfying assignments correspond to Hamiltonian cycles; hence 3-SAT $\preduce$ \text{HAM-CYCLE}.
\end{enumerate}

Traveling Salesperson (TSP-DEC) checks if a complete weighted graph $G=(V,E,w)$ has a tour of total weight $\leq B$:
\begin{enumerate}[\roman*.]
  \item NP: given tour $(v_1,\dots,v_n,v_1)$, check it visits each vertex once and compute $\sum w(v_i,v_{i+1}) \le B$ in $\Theta(|V|)$.
  \item NP-Hard: given HAM-CYCLE instance $G'=(V',E')$, build complete $G$ with
        $w(e)=1$ if $e\in E'$, and $w(e)=M\gg |V'|$ otherwise, and set $B=|V'|$; $G'$ has Hamiltonian cycle iff TSP-DEC has YES tour $\Rightarrow$ HAM-CYCLE $\preduce$ TSP-DEC.
\end{enumerate}

3-Dimensional Matching (3DM) has disjoint sets $X,Y,Z$ and collection $T \subseteq X\times Y\times Z$; it checks if there is a matching $M\subseteq T$ of size $k$ using each element at most once:
\begin{enumerate}[\roman*.]
  \item NP: given $M$, verify $|M|=k$ and no two triples in $M$ share an $X$, $Y$, or $Z$ element in $\Theta(k)$.
  \item NP-Hard: reduce 3-SAT to 3DM via gadgets for variables and clauses so that a satisfying assignment corresponds to a size-$k$ matching; hence 3-SAT $\preduce$ 3DM.
\end{enumerate}

\colbreak

Knapsack (0-1 Knapsack Decision) checks if there is a subset of $n$ items,
with values $v_i$ and weights $w_i$, whose total weight is $\le W$ and
total value is at least $V$:
\begin{enumerate}[\roman*.]
  \item NP: given a subset $X \subseteq [n]$, verify
        $\sum_{i\in X} w_i \le W$ and $\sum_{i\in X} v_i \ge V$
        in $\Theta(n)$ time.
  \item NP-Hard: reduce SUBSET-SUM to 0--1 Knapsack.  Given
        SUBSET-SUM instance $(S=\{a_1,\dots,a_n\}, T)$, create a
        Knapsack instance with $w_i = v_i = a_i$ for each $i$,
        capacity $W = T$ and target value $V = T$.  There is a subset
        of $S$ summing to $T$ iff there is a knapsack with weight
        $\le W$ and value $\ge V$:
        hence SUBSET-SUM $\preduce$ KNAPSACK.
\end{enumerate}
